...
/***/ function(module, exports) {
	...
	var ensureThat = exports.ensureThat = operation;
	...
	var EnsureThat = function () {
		...
		_createClass(EnsureThat, [{
			...
			key: "run",
			value: function run(routine, args) {
				this.routine = routine;
	
				for (var i = 0; i < this.operations.length; i++) {
					var _operation = this.operations[i];
					var result = _operation();
	
					if (result instanceof Promise) {
						var operationsToPromisify = this.operations.slice(i + 1, this.operations.length);
						return runAsynchronously(this, operationsToPromisify, result).then(function () {
							return args;
						});
					} else {
						if (!result) {
	...
	function runAsynchronously(runner, _operations, lastResult) {
		...
		try {
			for (var _iterator = operations[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
				var _operation2 = _step.value;
	
				promise = promise.then(_operation2).then(function (result) {
					if (!result) {
